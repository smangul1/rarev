...
for pileupcolumn in samfile.pileup("3"):
    ...
    if pileupcolumn.pos in snpSet:
    	if pileupcolumn.n>0:
            ...
            altCount=alleleList.count(snpDict[pileupcolumn.pos][0])
            
            
            if snpDict[pileupcolumn.pos][0] in allelesSet and snpDict[pileupcolumn.pos][1] not in allelesSet and len(allelesSet)==1:
                if altCount>10:
                    
                            #print allelesSet, len(alleleList),snpDict[pileupcolumn.pos]
                            fileWES.write("HOMO,"+str(pileupcolumn.pos)+","+str(snpDict[pileupcolumn.pos][0])+","+str(snpDict[pileupcolumn.pos][1])+","+str(altCount))
                            fileWES.write("\n")
        ...
        if pileupcolumn.n>0:
            ...
            for pileupread in pileupcolumn.pileups:
                if not pileupread.is_del and not pileupread.is_refskip:
                    ...
                    if pileupread.alignment.query_sequence[pileupread.query_position]!=snpDict[pileupcolumn.pos][0]:
                        #print ('\tbase in read %s = %s' % (pileupread.alignment.query_name, pileupread.alignment.query_sequence[pileupread.query_position]))
                                
                            out.write(str(pileupread.alignment)+","+str(pileupread.query_position)+","+str(pileupread.alignment.query_sequence[pileupread.query_position]) +","+ str(snpDict[pileupcolumn.pos][1])+","+ str(pileupread.alignment.query_qualities[pileupread.query_position]))
                            out.write("\n")
            ...
            ALT_count=alleleList.count(snpDict[pileupcolumn.pos][0])
